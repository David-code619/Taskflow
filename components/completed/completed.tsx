...
import { getCompletedTaskCount } from "@/lib/actions/task";
...
const tomorrow1000 = new Date(now);
tomorrow1000.setDate(tomorrow1000.getDate() + 1);
tomorrow1000.setHours(10, 0, 0, 0);
const nextDay2 = new Date(now);
nextDay2.setDate(nextDay2.getDate() + 2);
nextDay2.setHours(9, 0, 0, 0);
const nextDay3 = new Date(now);
nextDay3.setDate(nextDay3.getDate() + 3);
nextDay3.setHours(15, 0, 0, 0);
...
export default async function Completed() {
  const completedTasks = await getCompletedTaskCount();
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pt-4">
        <div className="space-y-1">
          ...
          <p className="text-base text-muted-foreground">
            Review your achievements and past completed tasks.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-emerald-500/10 border-2 border-emerald-500/20 p-6 md:p-8 rounded-[2rem] flex flex-col justify-between overflow-hidden relative">
          ...
          <div className="relative z-10">
            <h3 className="text-xs font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mb-1">
              Total Completed
            </h3>
            <p className="text-5xl lg:text-7xl font-black text-emerald-700 dark:text-emerald-300 tracking-tighter">
          ...
          <div className="mt-8 relative z-10 flex items-center gap-2 text-xs font-bold text-emerald-700 dark:text-emerald-400">
            <span className="bg-emerald-500/20 text-emerald-800 dark:text-emerald-200 px-2 py-1 rounded-md">
              +12%
            </span>
        ...
        <div className="bg-card border border-border p-6 md:p-8 rounded-[2rem] flex flex-col justify-between">
          <div>
            ...
            <p className="text-4xl lg:text-5xl font-black text-foreground tracking-tighter">
              {/* {tasks.length
        ...
        <div className="flex items-end justify-between mb-6">
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium text-muted-foreground tracking-wide">
              A record of your completed goals.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-3">
          {completedTasks?.completedTasks?.length === 0 ? (
            <div className="p-12 text-center border border-dashed border-border rounded-3xl bg-muted/20">
              <p className="font-mono uppercase tracking-widest text-sm text-muted-foreground">
            ...
            completedTasks?.completedTasks.map((task) => (
              ...
              >
                <div className="col-span-12 md:col-span-6 flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-base font-bold text-foreground line-through decoration-emerald-500/40 line-clamp-1">
                      {task.title}
                    </h4>
                    <p className="text-[10px] uppercase tracking-widest text-muted-foreground/70 font-bold mt-1 max-w-50 truncate">
                      {/* {task.duration
                ...
                <div className="col-span-12 md:col-span-1 flex items-center gap-1 justify-end md:opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity mt-2 md:mt-0 pt-3 md:pt-0 border-t border-border/50 md:border-0">
                  <button
                    ...
                    className="p-2.5 hover:bg-emerald-500/20 hover:text-emerald-600 dark:hover:text-emerald-400 rounded-xl transition-all cursor-pointer shadow-sm border border-transparent hover:border-emerald-500/30 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-emerald-500/20 focus:text-emerald-600"
                    title="Restore Task"
                  >
                  ...
                  <button
                    ...
                    className="p-2.5 hover:bg-destructive/10 hover:text-destructive rounded-xl transition-all cursor-pointer shadow-sm border border-transparent hover:border-destructive/30 focus:outline-none focus:ring-2 focus:ring-destructive focus:bg-destructive/10 focus:text-destructive"
